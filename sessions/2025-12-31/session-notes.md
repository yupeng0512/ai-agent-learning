...
**Q1: 为什么 in-batch negatives 方法需要较大的 batch_size？**
> A: batch_size 越大，负样本越多，对比学习训练越好。
> 补充：batch_size=32 有 31 个负样本，batch_size=256 有 255 个。负样本越多，模型越能区分"真正相似"和"看起来相似"。

**Q2: 温度参数 τ 设置为 0.01 和 0.5 时，模型学习行为有什么不同？**
> A: 0.01 时学习更精准挑剔，0.5 时学习更宽泛但可能不够精细。
> 补充：τ=0.01 时 softmax 接近 one-hot，只关注最相似样本；τ=0.5 时分布平滑，多个样本都有梯度贡献。τ 太小容易训练不稳定。

**Q3: 如果你的领域数据只有 500 条 (query, doc) 对，你会选择什么训练策略？**
> A: LoRA 微调。
> 补充：全量微调需 10k+ 数据防止过拟合，LoRA 只更新 ~1% 参数相当于正则化。另一选择：直接用通用模型 + 好的 Reranker。
